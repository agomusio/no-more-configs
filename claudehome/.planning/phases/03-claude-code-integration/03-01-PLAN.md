...
must_haves:
  truths:
    ...
    - "mcp-setup auto-runs on every devcontainer start via postStartCommand (zero manual setup for happy path)"
  ...
  artifacts:
    - path: ".devcontainer/devcontainer.json"
      provides: "MCP_GATEWAY_URL environment variable and postStartCommand auto-trigger"
  ...
  key_links:
    ...
    - from: ".devcontainer/devcontainer.json"
      to: "shell function (mcp-setup)"
      via: "postStartCommand invokes mcp-setup on every container start"
      pattern: "mcp-setup"
...
<objective>
Wire Claude Code to auto-connect to the MCP gateway by creating a shell function that generates `.mcp.json` at the workspace root, polls the gateway health endpoint, and runs automatically on every devcontainer start via postStartCommand. The shell function also remains available for manual re-runs after adding new servers.

Purpose: This is the final phase — making MCP tools available to Claude Code sessions without manual configuration. After this, any Claude Code session started in the devcontainer will have MCP tools available immediately with zero user intervention.

Output: Modified Dockerfile with shell function, updated devcontainer.json with MCP_GATEWAY_URL env var and postStartCommand auto-trigger, example server documentation alongside gateway config.
</objective>
...
<task type="auto">
  <name>Task 1: Add MCP_GATEWAY_URL env var, mcp-setup shell function, and postStartCommand auto-trigger</name>
  <files>.devcontainer/devcontainer.json, .devcontainer/Dockerfile</files>
  <action>
**devcontainer.json changes:**

1. Add `MCP_GATEWAY_URL` to the existing `containerEnv` section:
```json
...
2. Add `mcp-setup` to the existing `postStartCommand` chain. The current postStartCommand is:
```
sudo /usr/local/bin/init-firewall.sh && git config --global --add safe.directory /workspace && git config --global --add safe.directory '*' && git config --global core.autocrlf input
```

Append `&& mcp-setup` to the end of this command chain. This ensures mcp-setup auto-runs on every devcontainer start, providing zero manual setup for the happy path. The function is defined in the shell profile (added by Dockerfile below), so it will be available when postStartCommand runs.
...
- Add `mcp-setup` to `postStartCommand` in devcontainer.json so it auto-runs on every devcontainer start (user chose "Fully automatic" — the shell function still exists for manual refresh after adding servers)
...
5. Verify postStartCommand in devcontainer.json includes `mcp-setup` appended to the existing command chain
  </verify>
  <done>
- MCP_GATEWAY_URL is set in devcontainer.json containerEnv as "http://host.docker.internal:8811"
- postStartCommand includes `mcp-setup` so it auto-runs on every devcontainer start (zero manual setup)
...
1. **Auto-trigger on startup:** Verify devcontainer.json postStartCommand includes `mcp-setup` at the end of the command chain, ensuring automatic execution on every devcontainer start.

2. **Config generation:** The shell function (if manually invoked in current container) should generate a valid `.mcp.json` at `/workspace/.mcp.json`. Test by sourcing the function and running it:
   ```bash
...
3. **JSON validity:** The generated `.mcp.json` must be valid JSON with `mcpServers.mcp-gateway` containing `type: "sse"` and `url` ending in `/sse`.

4. **Health check behavior:** If gateway is running, function should succeed quickly. If not running, function should warn after ~30s but not exit with error.

5. **File inventory:**
   - `.devcontainer/devcontainer.json` has `MCP_GATEWAY_URL` in containerEnv AND `mcp-setup` in postStartCommand
...
- mcp-setup auto-runs on devcontainer start via postStartCommand (zero manual setup for happy path)
