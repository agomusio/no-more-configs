...
- [ ] **INFRA-01**: MCP gateway runs as a Docker Compose sidecar service in the existing Langfuse stack
- [ ] **INFRA-02**: Gateway uses correct image (`docker/mcp-gateway:latest`) with loopback-only port binding (`127.0.0.1:8811:8811`)
- [ ] **INFRA-03**: Gateway config is driven by `mcp.json` — adding a new MCP server requires only editing this file and restarting
- [ ] **INFRA-04**: Gateway health check endpoint (`/health`) passes and is used by Docker Compose for orchestration
- [ ] **INFRA-05**: No port conflicts with existing Langfuse services (3030, 3052, 5433, 6379, 8124, 9000, 9090, 9091)

### Filesystem MCP

- [ ] **FSMCP-01**: Filesystem MCP server is configured in `mcp.json` and started by the gateway via stdio transport
- [ ] **FSMCP-02**: Workspace content is mounted identically in both devcontainer and gateway (`/workspace`) to avoid path mismatch
- [ ] **FSMCP-03**: Filesystem MCP can read and write files in the workspace directory

### Security

- [ ] **SEC-01**: Gateway binds to `127.0.0.1` only — no LAN exposure
- [ ] **SEC-02**: Docker socket is NOT mounted by default; gated behind a disabled-by-default compose profile (`mcp-docker-tools`)
- [ ] **SEC-03**: API keys and secrets are passed via environment variables, not hardcoded in `mcp.json`
- [ ] **SEC-04**: Firewall (iptables) allows traffic to gateway port 8811 from devcontainer
