...
<task type="auto">
  ...
  <action>
    ...
    Verify DIR-03 (planning state at workspace root):
    - Confirm .planning/ exists at /workspace/ with current planning state
    - ls .planning/ROADMAP.md .planning/STATE.md .planning/PROJECT.md (all must exist)
    - ls .planning/phases/02-directory-dissolution/ (phase 2 planning directory must exist)
    - claudehome/.planning/ contains stale planning from a different project scope and will be deleted in Plan 02
    - DIR-03 is satisfied because workspace .planning/ already contains current planning -- no migration needed
  ...
  <done>Custom skills exist in agent-config/skills/, infrastructure stack exists in infra/, and verification scripts exist in infra/scripts/. Git history is preserved via git mv. DIR-03 satisfied: workspace .planning/ contains current Phase 2 planning state; claudehome/.planning/ (stale) will be deleted in Plan 02.</done>
</task>
