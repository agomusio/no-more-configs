...
2. **Launch from anywhere.** Claude Code sessions can be started from any directory in the container — not just a fixed launch directory.
...
```bash
# Dockerfile — aliases (previously forced cd to a fixed directory, now launch from cwd)
...
### Skills location
```
agent-config/skills/  (source, installed to ~/.claude/skills/)
├── devcontainer/
└── gitprojects/
```
...
- Project: generated by `install-agent-config.sh` from `agent-config/settings.json.template`
...
### What happened to the former launch directory

The former fixed launch directory has been dissolved. Its contents were redistributed:

- Skills moved to `agent-config/skills/` (source) and `~/.claude/skills/` (installed location).
- Infrastructure moved to `infra/` (formerly `langfuse-local/`).
- Planning files moved to `/workspace/.planning/`.
...
- **Update** the alias definitions to remove the fixed directory `cd` prefix.
...
- Move skills from their former location to `agent-config/skills/`.
