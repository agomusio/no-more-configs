...
Your `~/.claude/settings.json` needs the Langfuse hook registered. Use the example at `infra/settings-examples/global-settings.json` as a reference:
...
| `claudeyr`  | `claude --dangerously-skip-permissions --resume`                      |
| `mcp-setup` | Regenerates `/workspace/.mcp.json` and health-checks the MCP gateway  |

Both accept extra arguments (e.g. `claudey -p "do something"`). Claude Code launches from the current working directory.

---

## Custom Skills

Custom skills are defined in `agent-config/skills/` (source) and installed to `~/.claude/skills/` at container creation by `install-agent-config.sh`. They provide domain-specific knowledge to Claude Code sessions.
...
- **Gateway config:** `infra/mcp/mcp.json` (server definitions)
...
Example configurations for GitHub, PostgreSQL, and Brave Search are in [`infra/mcp/SERVERS.md`](infra/mcp/SERVERS.md).

1. Copy a server entry into `infra/mcp/mcp.json` under `mcpServers`
2. Add any required env vars to `infra/.env`
3. Restart the gateway:
   ```bash
   cd ${LANGFUSE_STACK_DIR:-/workspace/infra} && sudo docker compose restart docker-mcp-gateway
   ```
...
| `/workspace/.planning/` | GSD project planning (PROJECT.md, ROADMAP.md, phases, research) |
| `/workspace/agent-config/` | Version-controlled agent config source (skills, templates) |
| `/workspace/agent-config/skills/` | Custom skill source (devcontainer, gitprojects) |
| `/workspace/config.json` | Master non-secret settings (firewall, langfuse, agent, vscode, mcp) |
| `/workspace/infra/` | Langfuse + MCP gateway Docker Compose stack |
| `/workspace/infra/docker-compose.yml` | Infrastructure stack definition (8 services) |
| `/workspace/infra/.env` | Generated infrastructure credentials (git-ignored) |
| `/workspace/infra/mcp/` | MCP gateway server configuration |
| `/workspace/infra/scripts/` | Infrastructure and verification scripts |
...
| `/home/node/.claude/state/langfuse_state.json` | Incremental processing state |
| `/var/run/docker.sock` | Host Docker socket (bind-mounted) |
| `/usr/local/bin/init-firewall.sh` | Firewall whitelist script |
...
4. **Verify API keys match:**
   - The `LANGFUSE_SECRET_KEY` in `~/.claude/settings.json` must match `LANGFUSE_INIT_PROJECT_SECRET_KEY` in `/workspace/infra/.env`
...
│   └── setup-network-checks.sh # Langfuse + host reachability checks
│
...
│   ├── mcp-templates/          # MCP server templates with {{PLACEHOLDER}} tokens
...
│   │   └── SERVERS.md          # Example server configs
...
│   │   ├── verify-filesystem-mcp.sh    # MCP filesystem verification
│   │   └── verify-gateway-connectivity.sh  # Gateway connectivity test
...
│
└── docs/                       # Documentation, reviews, specs
```
